...
namespace projekpbobismillah.models
{
    public class Admin : User
    {
        ...
        public static string AmbilTotalBuku()
        {
            using (var conn = new Npgsql.NpgsqlConnection(dbConnString))
            {
                conn.Open();
                using (var cmd = new Npgsql.NpgsqlCommand("SELECT COUNT(*) FROM Buku", conn))
                {
                    return cmd.ExecuteScalar().ToString();
                }
            }
        }
        public static DataTable AmbilDataMemberPremium()
        {
            ...
            using (var conn = new Npgsql.NpgsqlConnection(dbConnString))
            {
                conn.Open();
                string query = @"
            SELECT TO_CHAR(start_date, 'Mon') AS bulan, COUNT(*) AS total 
            FROM Subscription 
        ...
        public static DataTable AmbilDataTotalMember()
        {
            DataTable dt = new DataTable();
            using (var conn = new Npgsql.NpgsqlConnection(dbConnString))
            {
                conn.Open();
                string query = @"
            SELECT TO_CHAR(start_date, 'Mon') AS bulan, COUNT(DISTINCT member_id) AS total 
            FROM Subscription 
            GROUP BY TO_CHAR(start_date, 'Mon'), EXTRACT(MONTH FROM start_date)
            ORDER BY EXTRACT(MONTH FROM start_date) ASC;";
        ...
        public static DataTable AmbilDataRevenue()
        {
            DataTable dt = new DataTable();
            using (var conn = new Npgsql.NpgsqlConnection(dbConnString))
            {
                conn.Open();
                string query = @"
            SELECT TO_CHAR(tgl_transaksi, 'Mon') AS bulan, SUM(harga) AS total 
            FROM Transaksi 
